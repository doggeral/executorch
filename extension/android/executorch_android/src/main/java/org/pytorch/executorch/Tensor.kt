...
internal constructor(shape: LongArray) {
  ...
  @DoNotStrip
  fun dtypeJniCode(): Int = dtype().jniCode
  ...
  /**
   * Copies the tensor's data into a caller-provided [FloatBuffer], avoiding the per-call
   * allocation that [dataAsFloatArray] performs.
   *
    ...
    val tensorAsByteArray: ByteArray =
        when (dtype()) {
          ...
          DType.DOUBLE -> {
            ...
            ByteBuffer.wrap(arr)
                .asDoubleBuffer()
                .put((this as Tensor_float64).dataAsDoubleArray)
            arr
          }
          else -> throw IllegalArgumentException("Unknown Tensor dtype")
        }
    val byteBuffer =
        ByteBuffer.allocate(1 + 1 + 4 * shape.size + dtypeSize * numel().toInt())
  ...
  internal class Tensor_float32 internal constructor(private val data: FloatBuffer, shape: LongArray) :
      Tensor(shape) {
  ...
  internal class Tensor_float16 internal constructor(private val data: ShortBuffer, shape: LongArray) :
      Tensor(shape) {
  ...
  internal class Tensor_float64 internal constructor(private val data: DoubleBuffer, shape: LongArray) :
      Tensor(shape) {
  ...
  internal constructor(
    ...
    init {
      Log.e(
          "ExecuTorch",
          "$this in Java. Please consider re-export the model with proper return type")
    }

    override fun dtype(): DType = mDtype

    override fun toString(): String =
        "Unsupported tensor(${Arrays.toString(shape)}, dtype=$mDtype)"
  }

  // endregion nested tensor types

  companion object {
    private const val ERROR_MSG_DATA_BUFFER_NOT_NULL = "Data buffer must be not null"
    private const val ERROR_MSG_DATA_ARRAY_NOT_NULL = "Data array must be not null"
    private const val ERROR_MSG_SHAPE_NOT_NULL = "Shape must be not null"
    ...
    fun fromBlobUnsigned(data: ByteBuffer, shape: LongArray): Tensor {
      ...
      checkArgument(
          data.order() == ByteOrder.nativeOrder(), ERROR_MSG_DATA_BUFFER_MUST_HAVE_NATIVE_BYTE_ORDER)
    ...
    fun ones(shape: LongArray, dtype: DType): Tensor {
      ...
      return when (dtype) {
        ...
        DType.DOUBLE -> fromBlob(DoubleArray(numElements) { 1.0 }, shape)
            ...
            throw IllegalArgumentException("Tensor.ones() cannot be used with DType $dtype")
      }
    }
    ...
    fun zeros(shape: LongArray, dtype: DType): Tensor {
      ...
      return when (dtype) {
        ...
        DType.DOUBLE -> fromBlob(DoubleArray(numElements), shape)
            ...
            throw IllegalArgumentException("Tensor.zeros() cannot be used with DType $dtype")
      }
    }
    ...
    fun fromByteArray(bytes: ByteArray): Tensor {
      require(bytes != null) { "bytes cannot be null" }
